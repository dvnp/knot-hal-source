...
static uint8_t enable_keepalive = 0;
...
static int check_keepalive(int spi_fd, int sockfd)
{
	...
	if (hal_timeout(hal_time_ms(), peers[sockfd-1].keepalive_wait,
		...
		err = write_keepalive(spi_fd, sockfd, NRF24_LL_CRTL_OP_KEEPALIVE_REQ);
...
static int read_raw(int spi_fd, int sockfd)
{
	...
	p.pipe = sockfd;
	// printf("checando se chegou em p.pipe %d\n",sockfd);
	/*
	...
	while((ilen = phy_read(spi_fd, &p, NRF24_MTU)) > 0) {
		...
		case NRF24_PDU_LID_CONTROL:
		{
			...
			if (ctrl->opcode == NRF24_LL_CRTL_OP_KEEPALIVE_REQ) {
				printf("CHEGOU NRF24_LL_CRTL_OP_KEEPALIVE_REQ\n");
				peers[sockfd-1].keepalive_wait = hal_time_ms();
				peers[sockfd-1].keepalive = 1;
				printf("SEND NRF24_LL_CRTL_OP_KEEPALIVE_RSP\n");
		...
		case NRF24_PDU_LID_DATA_END:
			...
			if (peers[sockfd-1].seqnumber_rx < ipdu->nseq)
				break; /* TODO: disconnect, data error!?!?!? Illegal byte sequence */
			...
			if (ipdu->lid == NRF24_PDU_LID_DATA_FRAG &&
				...
				break; /* TODO: disconnect, data error!?!?!? Not a data message */
...
static void running(void)
{
	...
	case RAW:
		...
		// printf("============================================================\n");
		// printf("sockIndex is %d and pipe is %d\n",sockIndex, peers[sockIndex-1].pipe);
		if (peers[sockIndex-1].pipe != -1) {
			// printf("CORRETOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO\n");
			read_raw(driverIndex, sockIndex);
			write_raw(driverIndex, sockIndex);

			/* If keepalive is enabled */
			/* Check if timeout occurred and generates disconnect event */
			if (enable_keepalive &&
				check_keepalive(driverIndex, sockIndex)  == -ETIMEDOUT &&
				...
				memcpy(evt_discon->src.address.b, aa_pipes[sockIndex-1],
					sizeof(aa_pipes[sockIndex-1]));
				evt_discon->dst.address.uint64 = addr_gw.address.uint64;
				printf("len_rx raw disconnect\n");
				peers[sockIndex-1].len_rx = sizeof(struct mgmt_nrf24_header) +
					sizeof(struct mgmt_evt_nrf24_disconnected);

				peers[sockIndex].pipe = -1;
...
ssize_t hal_comm_read(int sockfd, void *buffer, size_t count)
{
	...
	// if(sockfd == 1)
	// 	printf("peers[sockfd-1].len_rx %d\n",peers[sockfd-1].len_rx);
	...
	/* Returns the amount of bytes read */
		printf("hal_comm_read with length %d\n",length);
	return length;
}
...
int hal_comm_connect(int sockfd, uint64_t *addr)
{
	...
	printf("connect colocou pra enviar %d\n", connection_live);
	return 0;
}
